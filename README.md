...
>   }
>   ```
> - **`GET /todos/:id`** : Get an existing todo with id= ==:id==.
>
>   `Response` will be like
>   when calling `GET /todos/1`
>
>   ```json
>   {
>     "id": 1,
...
>   }
>   ```
>   if your data matches the validation
>   in `Response you get`
>   ```json
>   {
>     "id": 2,
...
>   }
>   ```
> - **`PUT /todos/:id`** : Update an existing todo with id= ==:id==.
>   Send `Request` with body like
>   ```json
>   {
>     "text": "hello world",
...
>   }
>   ```
> - **`DELETE /todos/:id`** : Delete an existing todo with id= ==:id==.
>
> ---
